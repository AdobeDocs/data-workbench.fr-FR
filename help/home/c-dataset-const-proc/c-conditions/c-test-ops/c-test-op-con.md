...
La condition [!DNL Compare] compare des valeurs string ou numériques. Pour les comparaisons de valeurs de chaîne, vous pouvez indiquer si la casse doit être prise en compte.
...
La condition [!DNL Range] utilise un champ de saisie et détermine si la valeur de ce champ est incluse, dans les valeurs de paramètre minimales (Min) et maximales (Max) données.
...
Cet exemple utilise une condition [!DNL Range] pour définir la condition [!DNL Log Entry Condition]. Comme le serveur Data Workbench lit chaque [!DNL event data] enregistrement, il compare les valeurs numériques x-age et 55. Si pour une entrée de journal donnée, x-age est d’au moins 55 ans, l’entrée de journal est incluse dans le processus de construction du jeu de données. Cet exemple exécute la même fonction que l’exemple de condition [!DNL Compare]. Voir [Comparaison](../../../../home/c-dataset-const-proc/c-conditions/c-test-ops/c-test-op-con.md#section-fb2bdb3838504099b324b9838cdeeaac).
...
Le test de condition [!DNL Regular Expression] utilise une correspondance de modèle d’expressions régulières (voir [Expressions régulières](../../../../home/c-dataset-const-proc/c-reg-exp.md#concept-070077baa419475094ef0469e92c5b9c)) pour déterminer si la valeur du champ d’entrée spécifié contient une chaîne qui correspond à l’un des modèles spécifiés dans le paramètre Correspond à.

Si l’entrée est un vecteur de chaînes, seule la première valeur du vecteur est utilisée pour le test. La condition [!DNL Regular Expression] effectue des comparaisons de chaîne complètes. Si vous souhaitez identifier les sous-chaînes, vous devez ajouter en préfixe et en suffixe &quot;.*&quot; à la chaîne.
  ...
  <tr> 
   ...
   <td colname="col2"> <p>Modèle(s) d’expression régulière à faire correspondre à la valeur du champ de saisie. </p> <p> <b> Pour ajouter un modèle d’expression régulière</b> 
     <ol id="ol_6D6467FF74334DEA8E8625C3B155D11D"> 
      <li id="li_9E13A63558FF44749C2E49BD50B7F770">Cliquez avec le bouton droit de la souris sur <span class="uicontrol"> Correspond à </span>. </li> 
...
La condition [!DNL String Match] teste l’égalité des chaînes. Il prend un champ spécifié comme entrée et teste la valeur de ce champ dans chaque entrée de journal par rapport aux chaînes spécifiées dans le paramètre Matches de l’opération. Si l’une de ces chaînes de correspondance sensibles à la casse est identique à la valeur du champ d’entrée fourni, l’opération renvoie true. Dans le cas où [!DNL StringCondition] ne contient aucune chaîne correspondante, la condition renvoie false. Si l’entrée est un vecteur de chaînes, seule la première valeur (chaîne) du vecteur est utilisée pour le test.
  ...
  <tr> 
   ...
   <td colname="col2"> <p>Chaîne(s) à associer à la valeur du champ de saisie. </p> <p> <b>Pour ajouter une chaîne</b> 
     <ol id="ol_9E32218C771445D88357960475FAD6EB"> 
      <li id="li_A700747858D0470491783E9B3933DAFE">Cliquez avec le bouton droit de la souris sur <span class="uicontrol"> Correspond à </span>. </li> 
...
Cet exemple utilise les données collectées à partir du trafic du site web pour illustrer l’utilisation de la condition [!DNL String Match]. La condition vérifie si le champ d’entrée (cs-uri-stem) correspond à l’une des deux chaînes spécifiées dans le paramètre Matches, et si le champ cs-uri-stem est la chaîne exacte [!DNL /navigation/footer.asp] ou la chaîne exacte [!DNL /navigation/header.asp].
