...
<table id="table_5FF491A7A6C24E43A06A5C344BF05430"> 
 ...
 <tbody> 
  ...
  <tr> 
   ...
   <td colname="col2"> <p>Permet de désactiver le suivi des visiteurs, qui peut être utilisé pour se conformer aux stratégies d’exclusion. </p> <p>Lorsqu’elle est activée, <span class="wintitle"> Sensor</span> ne consigne pas de "paquet" pour un visiteur dont le cookie V1st est défini sur le PrivacyID spécifié. Comme aucune information n’est consignée pour ces visiteurs, aucune information sur ces visiteurs n’est envoyée à la variable <span class="keyword"> serveur Data Workbench</span> pour le traitement. </p> <p>Pour activer cette fonctionnalité, vous devez effectuer les étapes suivantes : 
     <ol id="ol_5D658C5E4AB14F419029E0FFC52F1310"> 
      <li id="li_BF056439F92148169BF592731264C3CD"> PrivacyID doit être défini avec une valeur de 0 (zéro) dans la variable <span class="filepath"> txlogd.conf</span> pour le fichier <span class="wintitle"> Sensor</span>. <p>Exemple : <span class="filepath"> PrivacyID 0</span> </p> </li> 
  ...
  <tr> 
   ...
   <td colname="col2"> <p>Emplacement auquel l’émetteur (txlogd) envoie périodiquement des demandes pour voir si le site web fonctionne correctement. </p> <p>Notez que l’emplacement est spécifié au format suivant, et non en tant qu’URL : </p> <p>http,<i>serverAddress,port,/resource</i> </p> <p>where <i>serverAddress</i> est le nom du serveur ou l’adresse IP, <i>port</i> est le port d’écoute HTTP du serveur, et <i>resource</i> est la ressource spécifique à demander (peut inclure une chaîne de requête). </p> <p>Vous pouvez spécifier plusieurs lignes SiteTest. </p> <p>Exemple : <span class="filepath"> SiteTest http,localhost,80,/test.html</span> </p> <p> <p>Remarque : Seul http est actuellement pris en charge. </p> </p> </td> 
  </tr> 
  <tr> 
...
<table id="table_598C3CDA5AA440228AF88C3BE4A8F77C"> 
 ...
 <tbody> 
  <tr> 
   ...
   <td colname="col2"> <p>Définissez ce paramètre uniquement lorsque vous utilisez les services de conseil Adobe. </p> <p>Indique à IIS <span class="wintitle"> Sensor</span> Parmi les deux "hooks" de journalisation possibles, lequel doit être utilisé pour consigner un paquet </p> <p>Utilisez ce paramètre lorsque le service IIS <span class="wintitle"> Sensor</span> ne consigne pas correctement les paquets. Ce paramètre est défini sur "off" si le crochet de journalisation par défaut ne fonctionnait pas correctement. La valeur par défaut est "on". </p> <p>Exemple : <span class="filepath"> IISCaptureBytesEnvoyé sur</span> </p> </td> 
  </tr> 
  <tr> 
   <td colname="col1"> IISUseAlternateHandler </td> 
   <td colname="col2"> <p>Définissez ce paramètre uniquement lorsque vous utilisez les services de conseil Adobe. </p> <p>Indique la variable <span class="wintitle"> Sensor</span> parmi deux hooks possibles à utiliser pour définir le cookie v1st . </p> <p>Vous utilisez ce paramètre lors de l’IIS <span class="wintitle"> Sensor</span> ne définit pas correctement le cookie v1st. Ce paramètre est défini sur "oui" si le crochet par défaut ne définissait pas correctement le cookie v1st. La valeur par défaut est "non". </p> <p>Exemple : <span class="filepath"> IISUseAlternateHandler non</span> </p> </td> 
  </tr> 
  <tr> 
   <td colname="col1"> <p>NewUserCacheControl </p> <p>CacheControl </p> </td> 
   <td colname="col2"> <p>Par défaut, <span class="wintitle"> Sensor</span> envoie des en-têtes de réponse de contrôle du cache à chaque requête. Lorsque la fonction de contrôle du cache est activée <span class="wintitle"> Sensor</span> envoie un en-tête Expires avec la valeur Thu, 01 décembre 1994 16:00:00 GMT au navigateur. </p> <p>Vous pouvez modifier les chaînes de réponse selon vos besoins en modifiant les deux lignes suivantes dans le <span class="filepath"> txlogd.conf</span> fichier : </p> <p> <span class="filepath"> NewUserCacheControl</span> <i>&lt;<span class="filepath"> string1</span>&gt;</i> </p> <p> <span class="filepath"> CacheControl</span> <i>&lt;<span class="filepath"> string2</span>&gt;</i> </p> <p>Exemple : </p> <p> <span class="filepath"> NewUserCacheControl no-cache=Set-Cookie</span> </p> <p> <span class="filepath"> CacheControl private,max-age=0,must-revalidate</span> </p> <p>Pour désactiver l’envoi des en-têtes de réponse de contrôle du cache, saisissez un trait d’union pour chaque ligne, comme illustré ci-dessous : </p> <p> <span class="filepath"> NewUserCacheControl -</span> </p> <p> <span class="filepath"> CacheControl -</span> </p> </td> 
  </tr> 
 </tbody> 
</table>
