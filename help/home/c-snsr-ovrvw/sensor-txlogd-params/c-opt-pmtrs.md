...
<table id="table_5FF491A7A6C24E43A06A5C344BF05430"> 
 ...
 <tbody> 
  <tr> 
   ...
   <td colname="col2"> <p>Permet de filtrer les adresses IP spécifiées. </p> <p>Lorsque vous filtrez une adresse particulière, un "paquet" n’est pas consigné. Cette fonctionnalité élimine les agents internes ou surveillés avant le traitement des journaux, ce qui accélère le traitement des journaux et réduit les besoins en enregistrement de données. Vous pouvez utiliser des caractères génériques lors de la spécification des adresses. </p> <p>Exemple : <span class="filepath"> AddressFilter 10.0.0.000</span> </p> </td> 
  </tr> 
  <tr> 
  ...
  <tr> 
   ...
   <td colname="col2"> <p>Définissez ce paramètre uniquement lorsque vous travaillez avec Adobe Consulting Services. </p> <p>Active la journalisation du débogage pour le module Web et l’émetteur. </p> <p>Utilisez ce paramètre lorsque le capteur <span class="wintitle"> </span> ne fonctionne pas correctement. Une fois ce paramètre défini, vous devez créer un fichier vide à l’emplacement spécifié du chemin d’accès et lui accorder des droits d’écriture pour tous les utilisateurs. Par exemple (dans un shell unix sur le serveur Web) : 
     ...
     </ul> </p> <p>Vous devez activer la journalisation du débogage pendant une courte période, après quoi le fichier journal doit être envoyé aux services de conseil en Adobe pour être analysé. </p> <p>Exemple : <span class="filepath"> DebugLogPath /var/log/vslog.txt</span> </p> <p>L'Adobe recommande que ce paramètre soit d'abord défini dans un environnement de test afin de déterminer l'effet sur votre système. </p> </td> 
  </tr> 
  <tr> 
   ...
   <td colname="col2"> <p>Désactive le champ spécifié </p> <p>Les utilisateurs peuvent éliminer les champs qu’ils n’utilisent pas ou qu’ils ne souhaitent pas stocker. Si le champ utilise des valeurs de chaîne, sa désactivation transmet une chaîne vide. Si le champ prend des valeurs numériques, sa désactivation transmet le nombre zéro (0). Vous pouvez désactiver les champs suivants : 
     <ul id="ul_4537B345EFAE449A9946DA0B52EA5C43"> 
      ...
      <li id="li_6D647C845EB54B1B84C756DCDD7DD4CC">x-new-visiteur </li> 
      ...
      <li id="li_A10438D2DEBB4ADFB574BF7094F9F0C4">cs-uri-requête </li> 
      <li id="li_1D83BA59AC8543319D1966BB8ED1D931">s-dns </li> 
      <li id="li_34A23CE1944F4AEFBE7D74E8D6D5BEE6">cs(parrain) </li> 
  ...
  <tr> 
   ...
   <td colname="col2"> <p>Ressource qui, lorsqu’elle est demandée, génère un nouvel identifiant de suivi et place l’utilisateur dans un groupe d’expériences. </p> <p> <p>Remarque :  Cette ressource n’a pas besoin d’exister physiquement sur le serveur Web. </p> </p> <p>Exemple : <span class="filepath"> ExpCookieURL /setcookie.htm</span> </p> </td> 
  </tr> 
  <tr> 
  ...
  <tr> 
   ...
   <td colname="col2"> <p>Détermine si le premier clic de chaque nouvel utilisateur est consigné même si l'utilisateur demande un type de document qui est exclu par le paramètre ContentFilterExclude. </p> <p>La valeur par défaut est "no". </p> <p>En règle générale, les fichiers image sont exclus par le paramètre ContentFilterExclude. Si LogAllNewUsers est défini sur "yes" et que le premier document qu'un nouvel utilisateur obtient du serveur est une image, cette demande est consignée. Si le paramètre LogAllNewUsers est défini sur "no" ou non (et en supposant que les images soient filtrées par le paramètre ContentFilterExclude) et que le premier document qu’un nouvel utilisateur obtient du serveur est une image, cette demande n’est pas consignée. </p> <p>Exemple : <span class="filepath"> LogAllNewUsers no</span> </p> </td> 
  </tr> 
  <tr> 
   <td colname="col1"> MaxPageLoadTime </td> 
   <td colname="col2"> <p>durée, en secondes, pendant laquelle l'émetteur attend d'envoyer le lot de paquets suivant. </p> <p>La valeur par défaut est de 15. </p> <p>Exemple : <span class="filepath"> MaxPageLoadTime 15</span> </p> <p> <p>Remarque :  Ne modifiez pas cette valeur de paramètre sans contacter préalablement les services de conseil en Adobe. </p> </p> </td> 
  </tr> 
  <tr> 
   <td colname="col1"> PrivacyID </td> 
   <td colname="col2"> <p>Permet de désactiver le suivi des visiteurs, qui peut être utilisé pour se conformer aux stratégies d’exclusion. </p> <p>Lorsqu'il est activé, <span class="wintitle"> Sensor</span> ne consigne pas de "paquet" pour un visiteur dont le cookie V1st est défini sur l'ID de confidentialité spécifié. Comme aucune information n’est consignée pour ces visiteurs, aucune information sur ces visiteurs n’est envoyée au <span class="keyword"> serveur de l’outil de données </span> pour traitement. </p> <p>Pour activer cette fonctionnalité, vous devez effectuer les étapes suivantes : 
     <ol id="ol_5D658C5E4AB14F419029E0FFC52F1310"> 
      <li id="li_BF056439F92148169BF592731264C3CD"> PrivacyID doit être défini avec la valeur 0 (zéro) dans le fichier <span class="filepath"> txlogd.conf</span> pour le capteur <span class="wintitle"> </span>. <p>Exemple : <span class="filepath"> PrivacyID 0</span> </p> </li> 
      <li id="li_3E20F068C2F94512A92F284C80273B1C">Les propriétaires de sites Web doivent écrire du code pour définir des cookies visiteurs (V1st) de sorte que la valeur d'ID de cookie corresponde à la valeur de PrivacyID définie "<span class="filepath"> txlogd.conf</span>". </li> 
     </ol> </p> </td> 
  </tr> 
  <tr> 
   <td colname="col1"> SiteTest </td> 
   <td colname="col2"> <p>Emplacement auquel l’émetteur (txlogd) envoie périodiquement des demandes pour vérifier si le site Web fonctionne correctement. </p> <p>Notez que l’emplacement est spécifié au format suivant, et non en tant qu’URL : </p> <p>http,<i>serverAddress, port,/resource</i> </p> <p>où <i>serverAddress</i> correspond au nom du serveur ou à l’adresse IP, <i>port</i> au port d’écoute HTTP du serveur et <i>resource</i> à la ressource spécifique à demander (peut inclure une chaîne de requête). </p> <p>Vous pouvez spécifier plusieurs lignes SiteTest. </p> <p>Exemple : <span class="filepath"> SiteTest http, localhost,80, /test.html</span> </p> <p> <p>Remarque :  Pour le moment, seul http est pris en charge. </p> </p> </td> 
  </tr> 
  <tr> 
...
<table id="table_598C3CDA5AA440228AF88C3BE4A8F77C"> 
 ...
 <tbody> 
  <tr> 
   ...
   <td colname="col2"> <p>Définissez ce paramètre uniquement lorsque vous travaillez avec Adobe Consulting Services. </p> <p>Indique à IIS <span class="wintitle"> Sensor</span> lequel des deux "hameoks" de journalisation possibles doit être utilisé pour journaliser un paquet </p> <p>Utilisez ce paramètre lorsque IIS <span class="wintitle"> Sensor</span> ne consigne pas correctement les paquets. Ce paramètre est défini sur "off" si le hook de journalisation par défaut ne fonctionnait pas correctement. La valeur par défaut est "on". </p> <p>Exemple : <span class="filepath"> IISCaptureBytesEnvoyé sur</span> </p> </td> 
  </tr> 
  <tr> 
   <td colname="col1"> IISUseAlternateHandler </td> 
   <td colname="col2"> <p>Définissez ce paramètre uniquement lorsque vous travaillez avec Adobe Consulting Services. </p> <p>Indique au capteur <span class="wintitle"> </span> lequel des deux crochets possibles doit être utilisé pour définir le cookie v1st. </p> <p>Utilisez ce paramètre lorsque le capteur IIS <span class="wintitle"> </span> ne définit pas correctement le cookie v1st. Ce paramètre est défini sur "yes" si le hook par défaut ne définit pas correctement le cookie v1st. La valeur par défaut est "no". </p> <p>Exemple : <span class="filepath"> IISUseAlternateHandler no</span> </p> </td> 
  </tr> 
  <tr> 
   <td colname="col1"> <p>NewUserCacheControl </p> <p>CacheControl </p> </td> 
   <td colname="col2"> <p>Par défaut, <span class="wintitle"> Sensor</span> envoie des en-têtes de réponse de contrôle de cache à chaque requête. Lorsque la fonction de contrôle du cache est activée <span class="wintitle"> Sensor</span> envoie un en-tête Expires avec la valeur Thu, 01 déc 1994 16:00:00 GMT au navigateur. </p> <p>Vous pouvez modifier les chaînes de réponse selon vos besoins en modifiant les deux lignes suivantes dans le fichier <span class="filepath"> txlogd.conf</span> : </p> <p> <span class="filepath"> </span> <i>&lt;&gt; NewUserCacheControlstring1</span>&gt;</i><span class="filepath"> </span></p> <p> <span class="filepath"> </span> <i>&lt;&gt; CacheControlstring2</span>&gt;</i><span class="filepath"> </span></p> <p>Exemple : </p> <p> <span class="filepath"> NewUserCacheControl no-cache=Set-Cookie</span> </p> <p> <span class="filepath"> CacheControl private,max-age=0,must-revalidate</span> </p> <p>Pour désactiver l'envoi d'en-têtes de réponse de contrôle de cache, tapez un trait d'union pour chaque ligne comme illustré ci-dessous : </p> <p> <span class="filepath"> NewUserCacheControl -</span> </p> <p> <span class="filepath"> CacheControl -</span> </p> </td> 
  </tr> 
 </tbody> 
</table>
...
<table id="table_88696CCA93874BB683538BD614E07890"> 
 ...
 <tbody> 
  <tr> 
   ...
   <td colname="col2"> <p>Définissez ce paramètre uniquement lorsque vous travaillez avec Adobe Consulting Services. </p> <p>Indique au capteur <span class="wintitle"> </span> lequel des deux crochets possibles doit être utilisé pour définir le cookie v1st. </p> <p>Utilisez ce paramètre lorsque le cookie Apache <span class="wintitle"> Sensor</span> ne définit pas correctement le cookie v1st. Ce paramètre est défini sur "yes" si le hook par défaut ne définit pas correctement le cookie v1st. La valeur par défaut est "no". </p> <p>Exemple : <span class="filepath"> ApacheUseAlternateHandler no</span> </p> </td> 
  </tr> 
  <tr> 
   <td colname="col1"> ApacheUseTwoHandlers </td> 
   <td colname="col2"> <p>Définissez ce paramètre uniquement lorsque vous travaillez avec Adobe Consulting Services. </p> <p>Indique au capteur <span class="wintitle"> </span> de tenter de définir le cookie v1st dans les deux crochets. </p> <p>Utilisez ce paramètre lorsque le cookie Apache <span class="wintitle"> Sensor</span> ne définit pas correctement le cookie v1st. La valeur par défaut est "yes". </p> <p>Si la valeur est "yes" et que le cookie v1st n’est pas correctement défini dans le premier hook, le second est utilisé. Si la valeur est "no", vous devez définir le paramètre ApacheUseAlternateHandler pour indiquer quel hook utiliser pour définir le cookie v1st. </p> <p>Exemple : <span class="filepath"> ApacheUseTwoHandlers yes</span> </p> </td> 
  </tr> 
  <tr> 
   <td colname="col1"> <p>NewUserCacheControl </p> <p>CacheControl </p> </td> 
   <td colname="col2"> <p>Par défaut, <span class="wintitle"> Sensor</span> envoie des en-têtes de réponse de contrôle de cache à chaque requête. Lorsque la fonction de contrôle du cache est activée <span class="wintitle"> Sensor</span> envoie un en-tête Expires avec la valeur Thu, 01 déc 1994 16:00:00 GMT au navigateur. </p> <p>Vous pouvez modifier les chaînes de réponse selon vos besoins en modifiant les deux lignes suivantes dans le fichier <span class="filepath"> txlogd.conf</span> : </p> <p> <span class="filepath"> </span> <i>&lt;&gt; NewUserCacheControlstring1</span>&gt;</i><span class="filepath"> </span></p> <p> <span class="filepath"> </span> <i>&lt;&gt; CacheControlstring2</span>&gt;</i><span class="filepath"> </span></p> <p>Exemple : </p> <p> <span class="filepath"> NewUserCacheControl no-cache=Set-Cookie</span> </p> <p> <span class="filepath"> CacheControl private,max-age=0,must-revalidate</span> </p> <p>Pour désactiver l'envoi d'en-têtes de réponse de contrôle de cache, tapez un trait d'union pour chaque ligne comme illustré ci-dessous : </p> <p> <span class="filepath"> NewUserCacheControl -</span> </p> <p> <span class="filepath"> CacheControl -</span> </p> <p> <p>Remarque :  Adobe recommande de ne pas désactiver cette fonctionnalité. </p> </p> </td> 
  </tr> 
 </tbody> 
</table>
