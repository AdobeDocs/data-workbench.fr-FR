...
1. Si vous disposez d’un accès administrateur à vos serveurs web ou d’applications, accédez au dossier d’installation [!DNL Sensor] sur n’importe quelle machine [!DNL Sensor] de votre grappe web pour accéder au fichier [!DNL TestExperiment.xls]. Si vous ne disposez pas d’un accès administrateur, contactez votre gestionnaire de compte Adobe pour demander le fichier [!DNL TestExperiment.xls].
...
Voici un exemple de feuille de calcul [!DNL TextExperiment.xls] complétée :
...
Cet exemple indique que l’expérience &quot;New_Homepage&quot; commence le 1er juin 2006, se termine le 30 juin 2006 et contient une population témoin représentant 50 % des visiteurs et un groupe de test comptant 50 % des visiteurs, qui voient un contenu différent pour un URI.
...
Vous devez déployer le fichier de configuration de l’expérience sur chaque ordinateur de votre grappe web qui exécute une [!DNL Sensor] et qui diffuse les pages impliquées dans l’expérience. Vous pouvez le faire à l’aide d’une procédure manuelle ou de votre système de gestion de contenu existant.

**Pour déployer le contenu du test**

* Sur chaque application ou serveur web exécutant une balise [!DNL Sensor] qui diffuse les pages impliquées dans l’expérience, utilisez votre processus de publication existant pour déployer le contenu du test à l’emplacement approprié.

   Par exemple, si vous souhaitez publier la page du groupe de test [!DNL index2.asp] dans le dossier test de votre site web ( [!DNL mysite.com]), vous publiez le fichier sur [!DNL www.mysite.com/test].
