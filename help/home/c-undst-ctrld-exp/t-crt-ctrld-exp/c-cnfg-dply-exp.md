...
Pour configurer l’expérience, vous devez compléter la feuille de calcul de configuration de l’expérience fournie par l’Adobe (nommée [!DNL TestExperiment.xls] par défaut). Ce fichier configure [!DNL Sensor] l&#39;expérience et il s&#39;agit de la version Excel du fichier texte que vous avez spécifiée dans [Modification du paramètre](../../../home/c-undst-ctrld-exp/t-en-ctrld-exp/c-mod-expfile-prm.md#concept-25232b386a654870becc789d4f1fcc28)ExpFile.
...
1. Si vous disposez d’un accès administrateur à vos serveurs Web ou d’applications, accédez au dossier [!DNL Sensor] d’installation sur n’importe quel [!DNL Sensor] ordinateur de votre grappe Web pour accéder au [!DNL TestExperiment.xls] fichier. Si vous n’avez pas d’accès administrateur, contactez votre gestionnaire de compte d’Adobe pour demander le [!DNL TestExperiment.xls] fichier.

1. Ouvrez le [!DNL TestExperiment.xls] fichier (vous pouvez renommer ce fichier si nécessaire) et renseignez les champs suivants :
...
<table id="table_FDD6AE631C614F97AD7AE8829E53CCAC"> 
 ...
 <tbody> 
  ...
  <tr> 
   ...
   <td colname="col2"> <p>Date et heure auxquelles l’expérience doit se terminer. Lorsque la date et l’heure d’arrêt se produisent, <span class="wintitle"> Sensor </span> arrête d’envoyer les valeurs de cookie identifiées comme un groupe de test aux URI de test et envoie tous les cookies aux URI de Population témoin. </p> <p>Format : MM/JJ/AAAA H:MM </p> <p>Consultez les notes du champ <span class="wintitle"> Début </span> . </p> </td> 
  </tr> 
  <tr> 
...
1. Après avoir rempli les variables dans la feuille de calcul de configuration de l&#39;expérience, enregistrez les modifications, puis enregistrez le fichier au format texte délimité par des tabulations ( [!DNL *.txt]) en utilisant le nom que vous avez spécifié dans le paramètre ExpFile dans le fichier [!DNL Sensor] de configuration. See [Modifying the ExpFile Parameter](../../../home/c-undst-ctrld-exp/t-en-ctrld-exp/c-mod-expfile-prm.md#concept-25232b386a654870becc789d4f1fcc28).
...
## Déploiement du fichier de configuration et test du contenu {#section-34ff29649f584b93bc6129b75084b37c}

Vous devez déployer le fichier de configuration de l’expérience sur chaque ordinateur de votre grappe Web qui exécute une grappe [!DNL Sensor] et diffuse les pages impliquées dans l’expérience. Vous pouvez le faire en utilisant une procédure manuelle ou votre système de gestion de contenu existant.

**Pour déployer votre contenu de test**

* Sur chaque application ou serveur Web exécutant une page [!DNL Sensor] qui diffuse les pages impliquées dans l’expérience, utilisez votre processus de publication existant pour déployer le contenu du test à l’emplacement approprié.
