---
description: Les notes de mise à jour des outils de données version 6.4 comprennent les nouvelles fonctionnalités, les exigences de mise à niveau, les bogues corrigés et les problèmes connus.
title: Notes de mise à jour des outils de données version 6.4
...
Les notes de mise à jour des outils de données version 6.4 comprennent les nouvelles fonctionnalités, les exigences de mise à niveau, les bogues corrigés et les problèmes connus.
...
Les outils de données version 6.4 comprennent les nouvelles fonctionnalités suivantes :

| Fonctionnalités de DWB 6.4 | Description |
|--- |--- |
| [Exportation vers Analytics Core Services](/help/home/c-release-notes-insight/c-6-4/dwb-crs-integration.md) | La fonction d’exportation CRS (Customer Record Service) vous permet d’exporter les données des outils de données du tableau détaillé vers les services principaux d’Adobe Analytics afin de les intégrer aux fonctionnalités d’autres analyses, y compris les rapports et analyses. |
| [Assistant Installation de station de travail](/help/home/c-install-insight/install-setup/dwb-client-installer.md) | Installez les Outils de données (client) à l’aide d’un assistant d’installation pas à pas. Téléchargez les Outils de données, définissez des options, installez l’application et synchronisez-la avec les serveurs. |
| Amélioration de la configuration de la station de travail | Après l’installation de la station de travail, l’espace de travail Configurer les connexions au serveur s’ouvre et contient des informations supplémentaires sur la saisie des informations de connexion au serveur dans le fichier Insight.cfg. Vous pouvez également afficher l’état de la connexion à vos serveurs et sélectionner des profils. |
...
| [Assistant Limite de mesure](/help/home/c-get-started/c-vis/dwb-create-metricdim/dwb-create-metricdim.md) | Utilisez un assistant pas à pas pour faciliter la création de dimensions de mesures. |
| [Administration utilisateur de l’accès des membres du groupe](/help/home/c-inst-svr/c-admin-inst-svr/c-config-acs-ctrl/dwb-self-admin-member-access.md) | Les administrateurs peuvent permettre aux utilisateurs de la station de travail de gérer de manière partielle le contrôle d’accès des groupes personnalisés. |
| [Verrouillage des profils dans la station de travail](/help/home/c-get-started/c-admin-intrf/c-prof-mgr/dwb-profile-lock.md) | Vous pouvez empêcher que les profils ne soient remplacés par des utilisateurs travaillant dans les gestionnaires Profil, Dimensions, Rapports, Espaces de travail, Mesures ou Filtres. Dans le Gestionnaire de profils, enregistrez le fichier Internal.cfg dans un profil personnalisé afin d’empêcher le remplacement de plusieurs fichiers de profil de votre système sur le serveur. |
| [Nouvelles fonctionnalités de l’interface utilisateur](/help/home/c-adobe-data-workbench-dashboard/t-getting-started/c-dashboard-user-interface.md) | Outils de données version 6.4 ajoute de nouvelles icônes d’espace de travail, des info-bulles, des écrans de démarrage et un raccourci F1 à l’aide. Dans cette version, vous pouvez également ouvrir les fichiers journaux en sélectionnant Aide > Ouvrir le répertoire de suivi dans la barre d’outils. |
| Algorithme de mise en grappe mis à jour | Optimisation des attentes ajoutée à la fonction de mise en grappe. Il s’agit d’une fonctionnalité d’Adobe Analytics Premium. |
...
| Ventilation de la visualisation des fichiers | La visualisation des fichiers (Admin > Fichiers) pour les profils de base ne comprennent pas les répertoires plus volumineux (Journaux, Exportations et Recherches supprimés) lors de la création de rapports. Le rapport s’affiche ainsi plus rapidement.  Les répertoires plus volumineux disposent désormais de leurs propres rapports individuels (Admin / Fichiers d’exportation, Fichiers de recherche et Fichiers journaux). |
| Device Atlas avec cache en mémoire | Le fichier DeviceAtlas.bundle utilise désormais le cache en mémoire pour améliorer les performances des recherches. |
...
| Faire glisser des dimensions de l’outil de recherche vers un tableau détaillé | A partir de la station de travail, vous pouvez désormais faire glisser des dimensions de l’outil de recherche directement vers le tableau détaillé de l’espace de travail. |

>[!NOTE]
>
>Vous pouvez désactiver les icônes de la barre d’outils et revenir aux noms de texte. Ajoutez le **[!DNL Toolbar Icons = bool: false]** paramètre au fichier [](https://docs.adobe.com/content/help/en/data-workbench/using/client/c-insght-config-param.html)Insight.cfg. False désactive les icônes de l&#39;interface utilisateur du poste de travail et affiche les noms des menus dans le texte de la barre d&#39;outils.

## Device Atlas avec cache en mémoire{#device-atlas-with-in-memory-cache}
...
Par défaut, Device Atlas met en cache jusqu’à 100 000 agents utilisateur et leurs propriétés. Le cache LRU est entièrement autonome dans DeviceAtlas.bundle. Dès lors, toute version du serveur capable d&#39;utiliser un fichier d&#39;assemblage bénéficiera automatiquement des performances accrues dès que le nouveau fichier sera chargé.
...
À compter de la version 6.4 des Outils de données, vous pouvez remplacer la limite des éléments mis en cache par défaut de &quot;100000&quot; en modifiant sa valeur et en enregistrant le fichier. La valeur par défaut a été sélectionnée pour répondre à la plupart des besoins.
...
* **Ajoutez** des processus ****** exclus pour la protection des points de fin System Center ** MS sur les serveurs Windows 2012 pour les exécutables suivants :
   ...
   * **`ExportIntegration.exe`**
   Cela permettra d&#39;accorder des droits de liste blanche à ces exécutables interfacés.

* **Mettez à jour le *`Trust_ca_cert.pem`*certificat sur les serveurs**.
* **Réorganisation des profils** d’attribution.

   * Le dossier *Attribution* a été renommé ***Attribution - Premium*** (situé dans l’installation par défaut sous *Profils* \ *Attribution - Premium).*
...
* Dans le fichier **Log Processing.cfg** , le paramètre Taille *du* bloc sous Sources *de* journaux a été supprimé.
* Dans le fichier **Disk Files.cfg** , le paramètre *Détect Disk Corruption* a été supprimé aux emplacements suivants :
...
* La fonction *principale d’exportation* de profil marketing du tableau détaillé a été renommée ***Profils et audiences Exportation***.
* La fonction Exportation ** Test and Target du tableau détaillé a été renommée Exportation ****** Adobe Target.

## Corrections de bogues {#section-bcdd732af53249b893d30bb42eeb14fc}

Les correctifs suivants ont été apportés dans les Outils de données version 6.4 (depuis la version 6.31 des Outils de données).

* Le score de propension n’était pas réinitialisé lors de l’exécution de différentes entrées dans le même espace de travail. Il est désormais réinitialisé correctement.
* Aucune dimension dénombrable disponible lors de la première ouverture de la matrice de corrélation n’a été corrigée.
* L’exportation des segments Target échouait car le champ mboxPC était manquant. Ce problème est maintenant corrigé.
...
Les restrictions suivantes sont connues dans les Outils de données version 6.4.
...
* Dans *Profils et Exportation* d’audience, la saisie de caractères non autorisés (`[CR]` ou `[TAB]`) sous forme de noms de colonne génère des journaux incorrects, ce qui entraîne une mauvaise exportation des données.
* Dans les versions chinoise et japonaise, un problème de codage de caractères Unicode peut se produire dans le navigateur de chemins d’accès.
